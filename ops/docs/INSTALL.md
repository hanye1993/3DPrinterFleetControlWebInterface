...
| 芯片 | 安装包 |
|------|--------|
| Apple Silicon（M 系列） | `packages/macos-4.3.3-arm64.dmg` |
| Intel | `packages/macos-4.3.3-amd64.dmg` |
...
| 系统 | 安装包 | 安装命令 |
|------|--------|----------|
| Ubuntu / Debian（x86_64） | `ubuntu-4.3.3-amd64.deb` | `sudo dpkg -i ubuntu-*-amd64.deb` |
| CentOS / RHEL / Rocky（x86_64） | `centos-4.3.3-amd64.rpm` | `sudo yum install ./centos-*-amd64.rpm` |
...
3. DSM → 套件中心 → **手动安装** → 选 `packages/syno-4.3.3-x86_64.spk`（或对应架构）
