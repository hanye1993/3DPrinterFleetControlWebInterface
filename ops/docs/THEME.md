...
参考实现：仓库 **`assets/themes/fullsite_board/`**。
...
| 包 | 路径 | siteMode | 特点 |
|----|------|----------|------|
| `default` | `assets/themes/default/` | skin | 经典侧栏 + 卡片墙；多套 styles |
| `lineboard` | `assets/themes/lineboard/` | skin | workspace + table + split 登录 |
| `fullsite_board` | `assets/themes/fullsite_board/` | **full** | 整站 `.htm` 骨架 + islands |
...
安装：软件设置 → **主题** → 上传 ZIP → **启用**；内置包可点「同步」。

选型：

- 只换颜色 → 改 `styles` / CSS，保持引擎不变  
- 换结构但仍用 React 壳 → 改 `layout` / `deviceView` / `loginLayout`（参考 `lineboard` / `theme-sample`）  
- 主题完全掌控 DOM → `siteMode: full`（参考 `fullsite_board`）
...
1. 在 `assets/themes/your_id/` 编写 `theme.json` 与资源。
2. 软件设置 → 主题 → **同步内置**（写入 `data/themes/`）。
3. **启用**该包；在样式列表选 `styles[].id`。
4. 改 CSS：硬刷新。改 `theme.json` / `.htm`：再同步或重启后刷新。
