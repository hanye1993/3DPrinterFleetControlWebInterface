...
> **推荐新插件走微内核 v2.1**（`apiVersion: "2"`）。旧版 `main.js` 导出同名钩子（v1）经 CompatAdapter 仍可用。  
> 领域事件、HMAC 回调、cron、SQL migrations、`ctx.alert` / `ctx.auth` / `ctx.cache` → 见内核文档。
...
源码：`assets/examples/plugin-sample/`、`assets/examples/plugin-kernel-v2/`、`assets/examples/plugin-chamber-temp/`（仓温，可自行打包）。
...
{
  "apiVersion": "2",
  "requires": { "kernel": ">=2.1.0" },
  "capabilities": [
    "log", "config.vars", "hooks", "http.route", "http.callback", "http.fetch",
    "devices.read", "templates", "alert.dispatch", "users.read", "auth.login",
