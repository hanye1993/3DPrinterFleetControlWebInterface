...
### 安装包一览（官网最新；下表文件名以 5.0.2 为例）

| 平台 | 文件 | 说明 |
|------|------|------|
| **Windows** | `windows-5.0.2-amd64.exe` | 双击安装，可选目录；Win10/11 64 位，需管理员 |
| **macOS Apple Silicon** | `macos-5.0.2-arm64.dmg` | 打开 DMG → 运行 `install.command` |
| **macOS Intel** | `macos-5.0.2-amd64.dmg` | 同上 |
| **飞牛 fnOS x86** | `fnos-5.0.2-x86.fpk` | 应用中心先装 **Node.js 20/22**，再手动安装 fpk |
| **飞牛 fnOS ARM** | `fnos-5.0.2-arm.fpk` | 同上 |
| **群晖 x86_64** | `syno-5.0.2-x86_64.spk` | 套件中心先装 **Node.js 20/22**，再手动安装 spk |
| **群晖 aarch64** | `syno-5.0.2-aarch64.spk` | 同上 |
...
1. 右键 `windows-5.0.2-amd64.exe` → **以管理员身份运行**
