...
```bash
sudo dpkg -i hanyemonitor-4.1.15-ubuntu-amd64.deb
# 若提示依赖问题：
...
```bash
sudo yum install ./hanyemonitor-4.1.15-centos-amd64.rpm
# 或 Fedora / 新版：
sudo dnf install ./hanyemonitor-4.1.15-centos-amd64.rpm
```

### ARM64 服务器

使用文件名带 `arm64` 的 deb/rpm（如 `hanyemonitor-4.1.15-ubuntu-arm64.deb`）。
