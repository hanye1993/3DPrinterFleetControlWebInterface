...
export class DeviceHost {
  ...
  async deviceOp(req: {
    ...
    try {
      ...
      if (!d) return { ok: false, message: '设备不存在' }
      if (!isMoonrakerDevice(d)) {
        return { ok: false, message: '仅 Klipper/Moonraker 类设备支持文件操作' }
      }
  ...
  async batchPrint(payload: {
    ...
    for (const deviceId of payload.deviceIds) {
      ...
      try {
        ...
        const ctrl = await this.control(deviceId, {
          action: 'print_file',
          filename: payload.filename
        })
        if (!ctrl.ok) throw new Error(ctrl.message || '开打失败')
  ...
  async approvedPrint(req: {
    ...
    return this.control(req.deviceId, { action: 'print_file', filename: req.filename })
  }
