...
async function bootstrap(): Promise<void> {
  ...
  // 应用集市心跳校验（LICENSE_REQUIRED=1 时硬拦截；只认 deviceId + 登录用户）
  {
    const { assertLicensedOrExit } = await import('../main/license/marketVerify')
  ...
  // 批量复查已装插件/主题授权（内置跳过；市场 APP_NOT_FOUND 视为本地包跳过）；非正版禁用插件/切回默认主题；定时复查；切回已购账号时自动恢复
  {
    const { recheckInstalledLicenses, startLicenseRecheckLoop, createLicenseSideEffects } =
      await import('../main/license/licenseGate')
    const getInstalled = () => [
    ...
    const { onPirate, onAllow } = createLicenseSideEffects({
      dataRoot: DATA_ROOT,
      setPluginAvailable: async (id, available) => {
        await pluginManager.setAvailable(id, available)
      },
      getActiveThemeId: () => String(appSettings.uiThemePack || ''),
      setActiveTheme: async (id) => {
        await themeManager.setActive(id)
      }
    })
    void recheckInstalledLicenses({
      ...
      onPirate,
      onAllow
    }).catch((e) => console.error('[license] 启动批量检查失败', e))
